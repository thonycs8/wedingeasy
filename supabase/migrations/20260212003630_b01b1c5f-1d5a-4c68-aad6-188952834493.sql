-- Enable unaccent extension for accent-insensitive name matching
CREATE EXTENSION IF NOT EXISTS unaccent;

-- Update public_rsvp to match names without accent sensitivity
...
BEGIN
  ...
  -- Match using unaccent for accent-insensitive comparison
  UPDATE guests
  SET confirmed = _confirmed, updated_at = now()
  WHERE wedding_id = _wedding_id
    AND lower(unaccent(name)) = lower(unaccent(btrim(_guest_name)));
